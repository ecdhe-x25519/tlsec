...
pub fn handle_certificate_verify_server(
    mut ctx: &mut Context<ServerSide>,
...
fn handle_sr(cert_ext: StatusRequestPayload, ctx: &mut Context<ServerSide>) -> Result<(), Error> {
    Err(Error::Alert(AlertDescription::BadCertificate))
}

fn handle_sct(cert_ext: StatusRequestPayload, ctx: &mut Context<ServerSide>) -> Result<(), Error> {
    Err(Error::Alert(AlertDescription::BadCertificate))
}