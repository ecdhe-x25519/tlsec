...
impl Serialize for CertificatePayload {
    ...
    fn decode(buf: &mut BytesMut) -> TlsResult<Self> {
        ...
        if buf.remaining() < certificate_request_context_length + 3 {
            error!(format!("Incomplete data: need {} more bytes", (certificate_request_context_length + 3 - buf.remaining())));
            return Err(TlsError::Incomplete(certificate_request_context_length + 3 - buf.remaining()));
        }
        ...
        if buf.remaining() < certificate_list_length {
            error!(format!("Incomplete data: need {} more bytes", (certificate_list_length - buf.remaining())));
...
impl Serialize for CertificateEntryPayload {
    ...
    fn decode(buf: &mut BytesMut) -> TlsResult<Self> {
        ...
        if buf.remaining() < cert_length + 2 {
            error!(format!("Incomplete data: need {} more bytes", (cert_length + 2 - buf.remaining())));
            return Err(TlsError::Incomplete(cert_length + 2 - buf.remaining()));
        }
        ...
        if buf.remaining() < ext_length {
            error!(format!("Incomplete data: need {} more bytes", (ext_length - buf.remaining())));
...
impl TryFrom<u16> for CertificateEntryExtensionType {
    ...
    fn try_from(value: u16) -> TlsResult<Self> {
        match value {
            ...
            _ => {
                warn!("Unknown certificate extension");
                Err(TlsError::Unknown("certificate extension"))
            }
        }
    }
}
...
impl CertificateEntryExtensionPayload {
    ...
    pub fn decode_payload(extension_type: CertificateEntryExtensionType, buf: &mut BytesMut) -> TlsResult<Self> {
...
#[cfg(test)]
mod test_cert_parse {
    use crate::message::handshake::extensions::StatusType;
