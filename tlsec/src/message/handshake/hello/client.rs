...
impl Serialize for ClientHelloPayload {
    ...
    fn decode(buf: &mut BytesMut) -> TlsResult<Self> {
        ...
        } else if buf.remaining() < legacy_session_id_length + 2 {
            error!(format!("Incomplete data: need {} more bytes", (legacy_session_id_length + 2 - buf.remaining())));
            return Err(TlsError::Incomplete(legacy_session_id_length + 2 - buf.remaining()));
        }
        ...
        if buf.remaining() < cipher_suites_length {
            error!(format!("Incomplete data: need {} more bytes", (cipher_suites_length - buf.remaining())));
        ...
        if buf.remaining() < legacy_compression_methods_length {
            error!(format!("Incomplete data: need {} more bytes", (legacy_compression_methods_length - buf.remaining())));
